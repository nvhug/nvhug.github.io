...
export default function FundFutureClient() {
  ...
  if (!userId) {
    return (
      <main className="flex min-h-svh flex-col items-center justify-center bg-[#f7fef9] px-4">
        <div className={`${cardClass} w-full max-w-sm px-8 py-10 text-center`}>
          ...
          <a
            href="/login"
            className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-emerald-600 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-700 active:scale-[0.98]"
          >
    ...
    <main className="min-h-svh bg-[#f7fef9] pb-16 pt-24">
      <div className="mx-auto w-full max-w-6xl space-y-4 px-4 sm:px-6">
        ...
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <div className={`${cardClass} col-span-2 p-4 lg:col-span-1`}>
        ...
        <>
          ...
          {pendingReceived.length > 0 && (
            <div className="mt-3 space-y-1.5 sm:mt-3 sm:space-y-1.5">
              <p className="text-xs font-medium text-zinc-400">{vi ? 'Lời mời nhận được' : 'Invites received'}</p>
          ...
          {sent.length > 0 && (
            <div className="mt-3 space-y-1.5 sm:mt-3 sm:space-y-1.5">
              <p className="text-xs font-medium text-zinc-400">{vi ? 'Lời mời đã gửi' : 'Invites sent'}</p>
              {sent.map((item) => (
...
function CrudForm({ title, children, onSubmit, saving, onCancel, vi, inline = false }: { title: string; children: React.ReactNode; onSubmit: (event: React.FormEvent) => void; saving: boolean; onCancel: () => void; vi: boolean; inline?: boolean | 'transaction' }) {
  ...
  return (
    <form onSubmit={onSubmit} className={`grid gap-3 border-b border-emerald-100 bg-emerald-50/40 px-5 py-4 sm:grid-cols-2 ${inlineGridClass}`}>
      <p className={`text-xs font-semibold text-emerald-700 sm:col-span-2 ${inline ? 'lg:sr-only' : ''}`}>{title}</p>
      {children}
